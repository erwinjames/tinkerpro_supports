...
class ApiClient {
  ...
  // ── Chat-portal helpers (auto-add as_portal=1) ────────────────────────

  /// Same as [get] but stamps `as_portal=1` so chatResolveActor() routes
  /// us through the customer-shadow-user path even on a server that also
  /// has a coexisting staff session for the same browser/device cookie.
  Future<Map<String, dynamic>> getChat(String action,
      {Map<String, dynamic>? params}) {
    return get(action, params: {'as_portal': '1', ...?params});
  }

  Future<Map<String, dynamic>> postChat(String action,
      {Map<String, dynamic>? body}) {
    return post(action, body: {'as_portal': '1', ...?body});
  }
