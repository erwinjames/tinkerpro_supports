...
///
/// Usage: pull the singleton from [RingtoneService.instance]. The tones
/// only need to be built once for the lifetime of the app — [_init] is
/// idempotent and runs lazily on first start*/ping call.
class RingtoneService {
  ...
  Future<void> _init() {
    ...
    _initStarted = true;
    _initFuture = () async {
      // POTS-style two-tone ring: 440/480Hz double burst, then a long pause
      // so the loop sounds natural (not a continuous drone).
      _ringBytes = _wav([
      ...
      // Caller-side ringback: single tone on/off, longer silence.
      _ringbackBytes = _wav([
        const _Seg(440, 1.2), const _Seg(0, 2.8),
      ]);
      // Two-note chime for incoming chat messages (no loop).
      _pingBytes = _wav([
  ...
  /// Stops the looping ringtone/ringback. Does NOT stop an in-flight ping —
  /// pings are short and should always finish.
  ...
  // ── WAV synthesis ──────────────────────────────────────────────────────
  // 16-bit mono PCM at 22.05 kHz — plenty for the tones we need and keeps
  // the byte count small. Each segment is either a sine tone (freq > 0)
  // or silence (freq == 0). Tones get a short fade in/out so loop seams
  // and segment boundaries don't click.
  ...
  Uint8List _wav(List<_Seg> pattern) {
    ...
    for (final seg in pattern) {
      ...
      if (seg.freq <= 0) {
        // Silence — Int16List defaults to zero, just advance the cursor.
        idx += samples;
        continue;
      }
      final omega = 2 * pi * seg.freq / _sampleRate;
      final fade = min(samples ~/ 4, (_sampleRate * 0.01).round()); // 10ms
    ...
    out.add(_ascii('WAVE'));
    out.add(_ascii('fmt '));
    out.add(_le32(16));            // subchunk1 size for PCM
    out.add(_le16(1));             // audio format = PCM
    out.add(_le16(1));             // channels = mono
    out.add(_le32(_sampleRate));
    out.add(_le32(_sampleRate * 2)); // byte rate (16-bit mono → sr*2)
    out.add(_le16(2));             // block align
    out.add(_le16(16));            // bits per sample
    out.add(_ascii('data'));
...
class _Seg {
  ...
  final double freq;     // Hz; 0 = silence
